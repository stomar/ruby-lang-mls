...
describe MLS::Logger do
  ...
  it "can log an entry (error)" do
    ...
    _(@logger.entries.last).must_match /Error +\(ruby-talk, test error\) RuntimeError\z/
  end

  it "can return all entries in correct order" do
    _(@logger.entries.size).must_equal 6
    _(@logger.entries.first).must_match /2000-01-02 12:00:01.*test_first/
    _(@logger.entries.last).must_match  /2000-01-02 12:00:06.*test_last/
  end
  ...
  it "can return all errors in correct order" do
    ...
    _(@logger.errors.first).must_match /2000-01-02 12:00:03.*test_error/
    _(@logger.errors.last).must_match  /2000-01-02 12:00:04.*test_invalid/
  end
end
