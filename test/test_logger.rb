...
describe MLS::Logger do
  ...
  it "can log an entry (success)" do
    ...
    _(@logger.entries.last).must_match "Success (ruby-talk, test success)"
  end
  ...
  it "can log an entry (invalid)" do
    ...
    _(@logger.entries.last).must_match "Invalid (ruby-talk, test invalid)"
  end
  ...
  it "can log an entry (error)" do
    ...
    _(@logger.entries.last).must_match %r{Error +\(ruby-talk, test error\) RuntimeError\z}
  end

  it "can return all entries in correct order" do
    _(@logger.entries.size).must_equal 6
    _(@logger.entries.first).must_match %r{2000-01-02 12:00:01.*test_first}
    _(@logger.entries.last).must_match  %r{2000-01-02 12:00:06.*test_last}
  end
  ...
  it "can return recent entries in correct order" do
    ...
    _(@logger.recent_entries.last).must_match "ruby-talk, latest"
  end

  it "can return all errors in correct order" do
    _(@logger.errors.size).must_equal 2
    _(@logger.errors.first).must_match %r{2000-01-02 12:00:03.*test_error}
    _(@logger.errors.last).must_match  %r{2000-01-02 12:00:04.*test_invalid}
  end
end
